...
const EMPTY = { menu: DEFAULT_MENU, sales: [], expenses: [], inventory: [], staff: [], bills: [], shopping: [], recurring: [], money: { cashOpen: 0, bankOpen: 0, moves: [] }, users: [], sessionUserId: null };
const advance = (d, f) => {
...
export default function LemonDynastyManager() {
  ...
  const ACCESS = {
    admin: ["home", "sales", "bills", "money", "stock", "staff", "advisor"],
    manager: ["home", "sales", "bills", "money", "stock", "staff", "advisor"],
  ...
  const tabs = [
    ["home", "Home"], ["sales", "Sales"], ["bills", "Bills"], ["money", "Money"], ["stock", "Stock"], ["staff", "Staff"], ["advisor", "AI"],
  ];

  return (
    <div className={"ldm min-h-screen pb-20" + (dark ? " ldm-dark" : "")} style={{ background: "var(--bg)", color: "var(--ink)", fontFamily: "'DM Sans', ui-sans-serif, sans-serif" }}>
      ...
      {/* Bottom nav */}
      {me && (
      <div className="fixed bottom-0 left-0 right-0 border-t" style={{ background: "var(--card)", borderColor: "var(--border)" }}>
        <div className="max-w-2xl mx-auto flex">
          {tabs.filter(([k]) => allowed.includes(k)).map(([k, label]) => (
            <button key={k} onClick={() => setTab(k)} className="flex-1 py-3 font-medium disp"
              style={{ fontSize: 11, color: activeTab === k ? "var(--green)" : "var(--mut2)", borderTop: activeTab === k ? "3px solid #F3C623" : "3px solid transparent" }}>
              {label}
            </button>
          ))}
        </div>
    ...
    <div>
      <Card title="Bring in from WhatsApp">
        ...
        <div className="mt-3 pt-2 flex items-center justify-between" style={{ borderTop: "1px solid var(--border)" }}>
          ...
          <span className="text-xs" style={{ color: "var(--mut2)" }}>web v1</span>
        </div>
